...
def triton_layer_norm(x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-5):
    """
    Triton implementation of LayerNorm.
    
    This function expects a 3D input tensor (B, S, H) and applies
    LayerNorm over the last dimension (H).
...
def main():
    parser = argparse.ArgumentParser(description="Load config.yaml and run Triton LayerNorm kernel (3D)")
    ...
    try:
        # Note: LayerNorm is often applied to the full hidden dim, not just head_dim
        ...
        # Adjust key based on what's in config.yaml for layernorm
        # Using 'head_dim' as per the 'silu.py' template
        if 'head_dim' in op_config:
            HEAD_DIM = int(op_config['head_dim'])
        elif 'hidden_dim' in op_config: # Add fallback for common LayerNorm case
        ...
        # This logic is from silu.py, adjust if needed for layernorm
    ...
    print(f"Launching Triton LayerNorm for shape ({BATCH_SIZE}, {SEQ_LEN}, {HEAD_DIM})...")
    

    output_triton = triton_layer_norm(x, weight, eps)

    # 3. Verification
    print("Verifying result with PyTorch reference...")
    
    # Reference implementation using PyTorch
    layer = torch.nn.LayerNorm(HEAD_DIM, eps=eps).to(device)
    # Manually set weight to match Triton
    layer.weight.data = weight
    output_pytorch = layer(x)
